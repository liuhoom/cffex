...
const parseData = async (url, xmlString) => {
  xml2js.parseString(xmlString, (err, result) => {
    if (err) throw (url, err);
  ...
  //     await prisma.rtj.createMany({
  //       data: batch,
  //       skipDuplicates: true, // 如果需要跳过重复项
  //     });
  //   }
  // }

  // const batchSize = dailyList.length / 5 + 1; // 根据需要调整批量大小
  // await batchInsert(dailyList, batchSize);
